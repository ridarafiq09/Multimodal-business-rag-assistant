...
def insert_application_logs(session_id, user_query, model, gpt_response):
    conn = get_db_connection()
...
def get_latest_document_ids(limit=1):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id FROM document_store ORDER BY upload_timestamp DESC LIMIT ?",
        (limit,)
    )
